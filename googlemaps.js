...
function codeAddress() {
  ...
  //address += " Brighton and Hove"; //Bias, but not limit, results to our location
  geocoder.geocode( { 'address': address, region: 'GB'}, function(results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
      ...
      var addForDb = document.getElementById('id_address');
      var addForDisplay = document.getElementById('id_address_formatted');
      var addFromGoogle = results[0].formatted_address;
      addForDisplay.value = addFromGoogle.replace(/,\s/g, '\n');
      addForDb.value = addFromGoogle;
